				...
				try {
					...
					if (json.status === "success") {
						...
						if (data.totalItems > 0 && data.items) {
							...
							//Author
							if ((volume.authors !== null) && !$.trim($("#book_review_author").val())) {
							...
							//Genre
							if ((volume.categories !== null) && !$.trim($("#book_review_genre").val())) {
							...
							//Cover URL
							if ((volume.imageLinks !== null) && (volume.imageLinks.thumbnail !== null) && !$.trim($("#book_review_cover_url").val())) {
							...
							$(".error-details").text("A book with this ISBN was not found in the Google Books database.").show();
						}
					}
					else if (json.status === "error") {
						$(".error-details").html("Sorry, something went wrong. Please check to ensure that you have created a " +
							"Google API Key and that it has been entered correctly on the Book Review Settings page. " +
							"If you continue to experience problems, please <a href='mailto:donnapep@gmail.com'>email</a> " +
							"your Google API key and the ISBN that you are trying to retrieve book information for. Thanks!").show();
			...
			if (format !== "none") {
				...
				if (parts.length === 3) {
					...
					if (format === "short") {
						...
						releaseDate = months[currentMonth].substring(0, 3) + " " + currentDate + " " + currentYear;
					}
					else if (format === "long") {
						releaseDate = months[currentMonth] + " " + currentDate + ", " + currentYear;
					}
				}
			}
